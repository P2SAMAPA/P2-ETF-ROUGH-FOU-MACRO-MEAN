...
st.set_page_config(page_title="Rough fOU Process with Macro Mean", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Rough fOU Process with Macro Mean</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Fractional Ornstein‑Uhlenbeck with macro‑dependent long‑term mean</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Rough fOU")
...
st.sidebar.markdown(f"**Primary macro:** {config.PRIMARY_MACRO} | **Rough threshold H<{config.ROUGH_THRESHOLD}**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'rough_fou_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Macro‑conditional mean: {etf['fou_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Mean"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Mean", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Rough fOU Macro‑Conditional Mean (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Rough fractional Ornstein‑Uhlenbeck process** extends the standard OU with fractional Brownian motion (Hurst < 0.5).
        - The long‑term mean μ depends on macro variables (e.g., VIX): μ = β₀ + β₁·macro.
        - We estimate the mean‑reversion speed θ and macro sensitivity β₁ via regression on lagged log price and macro.
        - The score is the current macro‑conditional mean μ(macro_today).
        - High score → ETF is expected to drift upward given current macro.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the macro‑conditional means.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Rough fOU Process | Fractional Ornstein‑Uhlenbeck with macro mean")
