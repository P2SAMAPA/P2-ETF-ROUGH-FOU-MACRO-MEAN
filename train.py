...
from fou_process import rough_fou_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    macro_series = macro_window[config.PRIMARY_MACRO]
    if len(macro_series) < len(ret_window):
        return None
    raw_scores = {}
    for ticker in ret_window.columns:
        s = rough_fou_score(ret_window[ticker].values, macro_series.values)
        if not np.isfinite(s):
            s = 0.0
        raw_scores[ticker] = float(s)
    norm_scores = normalize_scores(raw_scores)
    sorted_norm = sorted(norm_scores.items(), key=lambda x: x[1], reverse=True)
    top_etfs = [{"ticker": t, "fou_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    results = {
        ...
        "rough_threshold": config.ROUGH_THRESHOLD,
        "universes": {}
    }
    ...
    out_file = f"output/rough_fou_{timestamp}.json"
